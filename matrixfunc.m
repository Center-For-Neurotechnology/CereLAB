function matrixfunc(frequencies,delays,intertrialinterval,numtrials,stimamplitude,stimchans,trainlength)
...
diary(['C:/Stimulation/MatrixDiary-' filename]);
...
res = configureStimulusPattern(cerestim, 1, 'AF', pulses, ...
        stimamplitude, stimamplitude, 90, 90, frequency, 53);
...
for tr = randidx
    ...
    if (freqs(tr) > 15)
        pulses = floor(freqs(tr)*trainlength/1000);
        frequency = freqs(tr);
        ...
        pulses = 1;
        frequency = 1000/(1000/freqs(tr) - floor(1000/freqs(tr))+1);
        res = configureStimulusPattern(cerestim, 3, 'AF', ...
            pulses, stimamplitude, stimamplitude, ...
            90, 90, frequency, 53);
        res = configureStimulusPattern(cerestim, 4, 'CF', ...
...
%     stimMatrix(trial,:) = [tr stimchans numtrials stimamplitude trainlength freqs(tr) dels(tr)];  
    stimMatrix(trial,:) = [trial stimchans numtrials stimamplitude trainlength freqs(tr) dels(tr)];
%     fprintf(logfile,'%d\t%d\t%d\n\r',trial,freqs(tr),dels(tr));
    fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n\r',trial,stimchans(1),stimchans(2),pulses,stimamplitude,trainlength,freqs(tr),dels(tr));
    fprintf(logfile,'\n\r');
    fprintf('Trial %d of %d, Frequency %d, Delay %d\n\n',trial,length(randidx),freqs(tr),dels(tr))
    trial = trial+1;
...
save(['C:/Stimulation/Matrix-',char(filename),'.mat'],'stimMatrix')
