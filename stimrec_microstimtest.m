...
folder=['D:\stimbackup\test\syd_stimulator'];
...
intertrialinterval = 2; % in seconds
...
logfile = fopen(['D:\stimbackup\test\Matrix-',num2str(stimamplitude/1000),'-ma', filename '.txt'], 'a');
...
%     tr = 32
    if (freqs(tr) > 15)
        ...
        res = configureStimulusPattern(cerestim, 4, 'CF', ...
            floor(freqs(tr)*trainlength/1000), stimamplitude, stimamplitude, ...
            90, 90, freqs(tr), 53);
%readSequenceStatus(cerestim)=0 now
        res = beginningOfSequence(cerestim); %readSequenceStatus(cerestim)=3 now
        res = beginningOfGroup(cerestim);%readSequenceStatus(cerestim)=3 now
        res = autoStimulus(cerestim, stimchans(1), 3);%readSequenceStatus(cerestim)=3 now
        res = autoStimulus(cerestim, stimchans(2), 4);%readSequenceStatus(cerestim)=3 now
        res = endOfGroup(cerestim);%readSequenceStatus(cerestim)=0 now
        res = endOfSequence(cerestim);%readSequenceStatus(cerestim)=0 now
