function stimchans = syncfunc_triplet(path, pairs, ntrials, intertrialinterval, amplitude, pulses,pulsewidth,frequency,interphase,trainlength,jitterinterval,intervalburstduration,delay)
...
fprintf(logfile,'\n\r');
fprintf(logfile,'\n\r');
fprintf(logfile,'Trial\tChannel1\tChannel2\tChannel3\tChannel4\tChannel5\tChannel6\tPulses\tAmplitude\tPulsewidth\tFrequency\tInterphase\tTrialInterval\n\r');
fprintf(logfile,'\n\r');

TrialTotal = ntrials*7;
currtrial = 1;
stimchans = zeros(TrialTotal,6);
% create base trial and iterate
for n = 1:ntrials
    % create permutation   
    posOrder = randperm(7);      % A, B, C, A+B, B+C, A+C, A+B+C
    for p=1:length(posOrder)
        ...
        if posOrder(p) <= 3 % A,B, C
            ...
            res = autoStimulus(cerestim, nullElectrode, 5);
            res = autoStimulus(cerestim, nullElectrode, 6);
            res = endOfGroup(cerestim);
            res = endOfSequence(cerestim);
            stimchans(currtrial,1:6) = [pairs(posOrder(p),1), pairs(posOrder(p),2), nullElectrode, nullElectrode, nullElectrode, nullElectrode];
            
            fprintf('Stimulating at pair %g - %g.\n',pairs(posOrder(p),1),pairs(posOrder(p),2));
            fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%g\n\r',n,pairs(posOrder(p),1),pairs(posOrder(p),2),'0','0', pulses,amplitude,pulsewidth,frequency,interphase,trialinterval);
        ...
        elseif posOrder(p) == 4 %A+B
            ...
            stimchans(currtrial,1:6) = [pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2), nullElectrode, nullElectrode];
            fprintf('Stimulating at pairs %g - %g  AND  %g - %g.\n',pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2));
            fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%g\n\r',n,pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2),'0','0',pulses,amplitude,pulsewidth,frequency,interphase,trialinterval);
            fprintf(logfile,'\n\r');
        ...
        elseif posOrder(p) == 5 %B+C
            ...
            stimchans(currtrial,1:6) = [pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2), nullElectrode, nullElectrode];
            fprintf('Stimulating at pairs %g - %g  AND  %g - %g.\n',pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2));
            fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%g\n\r',n,pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2),'0','0',pulses,amplitude,pulsewidth,frequency,interphase,trialinterval);
            fprintf(logfile,'\n\r');
        ...
        elseif posOrder(p) == 6 %A+C
            ...
            stimchans(currtrial,1:6) = [pairs(1,1),pairs(1,2),pairs(3,1),pairs(3,2), nullElectrode, nullElectrode];
            fprintf('Stimulating at pairs %g - %g  AND  %g - %g.\n',pairs(1,1),pairs(1,2),pairs(3,1),pairs(3,2));
            fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%g\n\r',n,pairs(1,1),pairs(1,2),pairs(3,1),pairs(3,2),'0','0',pulses,amplitude,pulsewidth,frequency,interphase,trialinterval);
            fprintf(logfile,'\n\r');


        elseif posOrder(p) == 7 %A+B+C
            res = beginningOfSequence(cerestim);
            res = beginningOfGroup(cerestim);
            res = autoStimulus(cerestim, pairs(1,1), 1);
            res = autoStimulus(cerestim, pairs(1,2), 2);
            ...
            stimchans(currtrial,1:6) = [pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2)];
            fprintf('Stimulating at pairs %g - %g  AND  %g - %g and %g - %g.\n',pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2));
            fprintf(logfile,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%g\n\r',n,pairs(1,1),pairs(1,2),pairs(2,1),pairs(2,2),pairs(3,1),pairs(3,2),pulses,amplitude,pulsewidth,frequency,interphase,trialinterval);
            fprintf(logfile,'\n\r');
        end
        % RUN
        res = cerestim.play(1);
